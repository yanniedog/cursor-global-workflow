...
function printReport(result, { quiet = false } = {}) {
  ...
  console.error('');
  console.error('Delegate fixes: pr-fix-agent (threads/CI). Re-audit: npm run pr:gates:check');
}
...
Use this script for merge-readiness, not closeout idle detection.`);
  ...
  const runOnce = () => evaluateGates(prNumber, { skipFeedbackPlan: args.skipFeedbackPlan });
